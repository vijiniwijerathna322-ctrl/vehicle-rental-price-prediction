...
def overview(d,all):
 ...
 with b:
  title("Listing activity","Average daily rate grouped by renter trip history.");activity=d.assign(trip_band=pd.cut(d.renter_trips_taken,[-1,0,5,20,50,100,float("inf")],labels=["0","1–5","6–20","21–50","51–100","100+"])).groupby("trip_band",observed=False)[TARGET].mean();st.bar_chart(activity,color="#2ca02c")
...
def data_page(raw,d):
 ...
 m=raw.isna().sum().rename("missing_values").reset_index().rename(columns={"index":"feature"});m["missing_rate"]=m.missing_values/len(raw);st.dataframe(m.sort_values("missing_values",ascending=False),hide_index=True,use_container_width=True,column_config={"missing_rate":st.column_config.NumberColumn("Missing rate",format="%.1%%")})
 with st.expander("Feature dictionary"):st.dataframe(pd.DataFrame([("daily_rate","Target","Daily listing price in USD"),("vehicle_*","Vehicle","Make, model, type and year"),("rating, review_count, renter_trips_taken","Activity","Rating and historical activity"),("location_*","Location","City, state and coordinates"),("fuel_type","Vehicle","Reported fuel category")],columns=["Feature(s)","Category","Description"]),hide_index=True,use_container_width=True)
 cols=["vehicle_make","vehicle_model","vehicle_type","fuel_type","vehicle_year","location_state","rating","renter_trips_taken",TARGET];title("Filtered listing data","Download the current selection for further review.");st.download_button("Download filtered data (CSV)",d[cols].to_csv(index=False).encode(),"vehicle_rental_filtered_data.csv","text/csv");st.dataframe(d[cols],hide_index=True,use_container_width=True,height=340)
def eda(d):
 ...
 with b:title("Rate by vehicle type","Average rate, by vehicle type.");st.bar_chart(d.groupby("vehicle_type")[TARGET].mean().sort_values(ascending=False),color="#2ca02c")
 ...
 title("Numeric correlations","Pearson correlations show linear association only, not causation.");n=[TARGET,"rating","renter_trips_taken","review_count","vehicle_year","vehicle_age","location_latitude","location_longitude"];st.dataframe(d[n].corr().style.format("{:.2f}").background_gradient(cmap="RdBu",axis=None),use_container_width=True)
...
def predict(m,o):
 ...
 with st.form("prediction"):
  ...
  with b:
   st.markdown("#### Listing and location");status=st.radio("Rating status",["Rated","Unrated"],horizontal=True);has=int(status=="Rated");rating=st.slider("Vehicle rating",1.,5.,4.9,.01,disabled=not has)if has else 0.;trips=st.number_input("Renter trips taken",o["renter_trips_taken"]["minimum"],o["renter_trips_taken"]["maximum"],o["renter_trips_taken"]["default"],step=1);state=st.selectbox("State",o["location_state"]);x,y=st.columns(2);lat=x.number_input("Latitude",o["location_latitude"]["minimum"],o["location_latitude"]["maximum"],o["location_latitude"]["default"],format="%.4f");lon=y.number_input("Longitude",o["location_longitude"]["minimum"],o["location_longitude"]["maximum"],o["location_longitude"]["default"],format="%.4f")
  go=st.form_submit_button("Estimate daily rental price",type="primary",use_container_width=True)
 if go:
  ...
  try:v=float(m.predict(row)[0]);st.metric("Estimated daily rental price",f"${v:,.2f}");st.caption("Historical-data model estimate—not a guaranteed market price.");st.dataframe(row,hide_index=True,use_container_width=True)
...
def main():
 st.markdown("""<style>
.stApp{background:#0E1117;color:#E5E7EB}.block-container{max-width:1120px;padding:3.25rem 3.5rem 3rem}h1,h2,h3{color:#F8FAFC!important;letter-spacing:-.02em}.dashboard-hero{padding:1.1rem 0 2.4rem;margin:0;text-align:center;background:none;color:#94A3B8;box-shadow:none}.dashboard-hero h1{font-size:2rem!important;font-weight:750;background:linear-gradient(90deg,#1f77b4,#2ca02c);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin:0 0 .65rem}.dashboard-hero p{max-width:720px;margin:auto;font-size:.92rem;line-height:1.65;color:#94A3B8}.dashboard-hero .eyebrow{display:none}[data-testid='stMetric']{background:#F8FAFC;border:1px solid #D8DEE8;border-top:3px solid #1f77b4;border-radius:12px;padding:1.2rem 1rem;box-shadow:0 4px 14px rgba(0,0,0,.22)}[data-testid='stMetric']:nth-child(2){border-top-color:#2ca02c}[data-testid='stMetric']:nth-child(3){border-top-color:#ff7f0e}[data-testid='stMetric']:nth-child(4){border-top-color:#1f77b4}[data-testid='stMetricLabel']{font-size:.7rem;text-transform:uppercase;letter-spacing:.06em;color:#64748B!important}[data-testid='stMetricValue']{font-size:1.45rem;color:#1E293B!important}section[data-testid='stSidebar']{background:#262730;border-right:1px solid #30323D}section[data-testid='stSidebar'] *{color:#F8FAFC!important}section[data-testid='stSidebar'] h1{font-size:1.15rem!important;margin-top:1.3rem}section[data-testid='stSidebar'] label{font-size:.9rem!important;padding:.28rem 0}section[data-testid='stSidebar'] [data-testid='stWidgetLabel']{color:#B8C1CF!important}.stButton>button,.stFormSubmitButton>button{background:linear-gradient(90deg,#1f77b4,#2ca02c)!important;border:0!important;border-radius:8px!important;color:#fff!important;font-weight:650!important}.stCaption,p,li{color:#AAB4C3!important}.stAlert{border-radius:8px}[data-testid='stDataFrame']{border:1px solid #303744;border-radius:8px;overflow:hidden}.dashboard-footer{margin-top:2.5rem;padding:1.25rem;text-align:center;background:#171B24;border-top:2px solid #1f77b4;border-radius:8px;color:#9AA5B5;font-size:.85rem}.dashboard-footer strong{color:#F8FAFC}@media(max-width:800px){.block-container{padding:1.4rem}.dashboard-hero h1{font-size:1.6rem!important}}</style>""",unsafe_allow_html=True)
